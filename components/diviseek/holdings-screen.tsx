...
type HoldingWithStock = Holding & { stock?: { price?: number | null; dividendYield?: number | null; divGrowth1y?: number | null; marketCap?: number | null } | null }
...
export function HoldingsScreen({
  ...
  const [list, setList] = useState<HoldingWithStock[]>(initialHoldings)
  ...
  return (
    <div className="relative min-h-screen">
      {/* Sticky header */}
      <header className="sticky top-0 z-30 border-b border-border-subtle bg-nav-bg px-5 pb-4 pt-8">
        ...
        <div className="mt-3 flex justify-end">
          <button
            onClick={cycleSort}
            className="inline-flex items-center gap-1.5 rounded-full border border-border-subtle bg-surface-subtle px-3 py-1.5 text-xs font-medium"
          >
      ...
      {loading ? (
        <div className="flex flex-col gap-2.5 px-5 py-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="flex items-center gap-3 rounded-2xl border border-border-subtle bg-card p-3.5">
        ...
        <ul className="grid grid-cols-1 md:grid-cols-2 gap-2.5 px-5 py-4">
          {sorted.map((h) => (
...
function Stat({ label, value, gold }: { label: string; value: string; gold?: boolean }) {
  return (
    <div className="rounded-xl border border-border-subtle bg-card px-2.5 py-2">
...
function HoldingRow({ holding, onToggleDrip, currency }: { holding: Holding & { stock?: { price?: number | null; dividendYield?: number | null; divGrowth1y?: number | null; marketCap?: number | null } | null }; onToggleDrip: () => void; currency: string }) {
  ...
  return (
    <li className="relative overflow-hidden rounded-2xl">
      ...
      <div className="absolute inset-y-0 left-0 flex w-24 flex-col items-center justify-center gap-1.5 rounded-2xl bg-card">
        <button
          ...
          className={cn(
            ...
            holding.drip ? "bg-[color:var(--success)]" : "bg-toggle-off",
          )}
        >
          <span
            className={cn(
              "absolute top-0.5 flex size-5 items-center justify-center rounded-full bg-white shadow-sm transition-all",
      ...
      <div
        ...
        className={cn(
          "flex items-center gap-3 rounded-2xl border border-border-subtle bg-card p-3.5",
      ...
      >
        ...
        <div className="text-right">
          ...
          {holding.stock?.price && (
            <p className="mt-0.5 text-[11px] tabular-nums text-muted-foreground">${holding.stock.price.toFixed(2)}</p>
          )}
          {holding.stock?.divGrowth1y != null && (
            <p className="text-[10px] tabular-nums" data-positive={holding.stock.divGrowth1y > 0}>
              {holding.stock.divGrowth1y > 0 ? "+" : ""}{holding.stock.divGrowth1y}%
            </p>
          )}
